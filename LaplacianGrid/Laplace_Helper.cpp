...
{  
    ...
    for(int index=index_start;index<=index_end;index++) {
      ...
      ComputeShadowGrid(offset_grid_ptr,packed_offset);
      ...
      for(int j=ymin;j<=ymax;j++)
      {
            ...
            double result=(double)0.;
              ...
              result += ((double)(*reinterpret_cast<T*>(y_base_addr + o_grid[i-1][j])) - y1_in[cur_index]);
              ...
              result += ((double)(*reinterpret_cast<T*>(y_base_addr + o_grid[i][j-1])) - y1_in[cur_index]);
    ...
    for(int index=index_start;index<=index_end;index++) {
      ...
      for(int k=zmin;k<=zmax;k++)
      {
            ...
            double result=(double)0.;
              ...
              result += ((double)(*reinterpret_cast<T*>(y_base_addr + o_grid[i-1][j][k])) - y1_in[cur_index]);

              result += ((double)(*reinterpret_cast<T*>(y_base_addr + o_grid[i+1][j][k])) - y1_in[cur_index]);

            
            
              result += ((double)(*reinterpret_cast<T*>(y_base_addr + o_grid[i][j-1][k])) - y1_in[cur_index]);
              ...
              result += ((double)(*reinterpret_cast<T*>(y_base_addr + o_grid[i][j][k-1])) - y1_in[cur_index]);
